...
## 故事回合兼容策略（2026-02）
1. 回合导演逻辑保持“版本无关”主干：非崇山故事统一走通用动态分支（结局分支与 plotItem 识别均按故事上下文驱动）。
2. 崇山医院启用专属兼容锁（`storySlug=chongshan-hospital` 或标题命中“崇山医院”）：
   - 终章冲刺阶段固定使用旧版 3 条结局选项文案，保持旧版节奏与叙事锚点。
   - `plotItem` 识别固定白名单：`病历/档案/录音/工牌/徽章/封印/阵列/蓝衣/赵医生/裂缝/守则原件`。
3. 该兼容锁只影响崇山医院，不影响新故事（如雾隐山庄）的生成与结局分支。
