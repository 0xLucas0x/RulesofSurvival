...
export const MainDisplay: React.FC<MainDisplayProps> = ({ 
  ...
  const encodedPrompt = encodeURIComponent(imagePrompt + ", horror style, cinematic lighting, gritty, dark atmosphere, 8k, photorealistic, night vision or surveillance camera style");
  const imageUrl = `https://image.pollinations.ai/prompt/${encodedPrompt}?width=1280&height=720&nologo=true&seed=${Math.random()}`;
    ...
    <section className="flex-1 flex flex-col relative overflow-hidden bg-black h-full font-sans">
      ...
      <div className="absolute inset-0 z-0 select-none">
        <img 
          ...
          src={imageUrl} 
          alt="Environment" 
          className={`w-full h-full object-cover transition-opacity duration-1000 ${isLoading ? 'opacity-40 blur-sm scale-105' : 'opacity-60 scale-100'}`}
          style={{ filter: "contrast(120%) saturate(40%) sepia(30%)" }}
        />
